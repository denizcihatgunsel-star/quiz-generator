    ...
    <div className="min-h-screen bg-[#f5f5f0]">
      ...
      <main className="max-w-5xl mx-auto px-4 py-10">
        ...
        <div className="mb-8">
          ...
          <p className="text-zinc-500 mt-1">Your quiz history and performance overview.</p>
        </div>

        {/* Stats Grid */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10">
          <div className="p-5 rounded-2xl bg-zinc-900/80 border border-zinc-800 card-glow">
            <p className="text-xs text-zinc-500 uppercase tracking-widest mb-2">Total Quizzes</p>
            <p className="text-3xl font-bold text-white">{totalQuizzes}</p>
          </div>
          <div className="p-5 rounded-2xl bg-zinc-900/80 border border-zinc-800 card-glow">
            <p className="text-xs text-zinc-500 uppercase tracking-widest mb-2">Completed</p>
            <p className="text-3xl font-bold text-white">{scoredQuizzes.length}</p>
          </div>
          <div className="p-5 rounded-2xl bg-zinc-900/80 border border-zinc-800 card-glow">
            <p className="text-xs text-zinc-500 uppercase tracking-widest mb-2">Avg Score</p>
          ...
          <div className="p-5 rounded-2xl bg-zinc-900/80 border border-zinc-800 card-glow">
            <p className="text-xs text-zinc-500 uppercase tracking-widest mb-2">Best Score</p>
            <p className="text-3xl font-bold text-white">
              {bestScore !== null ? (
        ...
        {/* Score Trend */}
        {scoreTrend.length >= 2 && (
          <div className="mb-10 p-6 rounded-2xl bg-zinc-900/80 border border-zinc-800">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-semibold text-white uppercase tracking-widest">Score Trend</h2>
              <span className="text-xs text-zinc-500">Last {scoreTrend.length} quizzes</span>
            </div>
        ...
        {filteredQuizzes.length === 0 ? (
          <div className="text-center py-16 rounded-2xl bg-zinc-900/80 border border-zinc-800">
          ...
          <div className="space-y-3">
            {filteredQuizzes.map((q) => {
              ...
              return (
                <div
                  key={q.id}
                  className="group p-5 rounded-2xl bg-zinc-900/80 border border-zinc-800 card-glow flex items-center justify-between gap-4"
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {q.shareId && (
                      <button
                        ...
                        className="px-3 py-1.5 rounded-full border border-zinc-800 text-xs text-zinc-400 hover:text-white hover:border-zinc-600 transition-colors"
                      >
                    ...
                    <Link
                      href={`/quiz/${q.shareId}`}
                      className="px-3 py-1.5 rounded-full bg-violet-600/10 border border-violet-500/20 text-xs text-violet-400 hover:bg-violet-600/20 font-medium transition-colors"
                    >
                      View
                    </Link>
                    {confirmDelete === q.id ? (
                      <div className="flex items-center gap-1">
                        <button
                          ...
                          className="px-2.5 py-1.5 rounded-full bg-red-500/10 border border-red-500/30 text-xs text-red-400 hover:bg-red-500/20 font-medium transition-colors disabled:opacity-50"
                        >
                          {deleting === q.id ? "..." : "Confirm"}
                        </button>
                        <button
                          onClick={() => setConfirmDelete(null)}
                          className="px-2.5 py-1.5 rounded-full border border-zinc-800 text-xs text-zinc-500 hover:text-zinc-300 transition-colors"
                        >
                          Cancel
                        </button>
                      </div>
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 rounded-full border border-zinc-800 text-xs text-zinc-600 hover:text-red-400 hover:border-red-500/30 transition-colors"
                      >
                        Delete
                      </button>
                    )}
